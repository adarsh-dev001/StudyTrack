...
import { Timer, HelpCircle, SkipForward, Star, ArrowLeft, Lightbulb, Image as ImageIcon } from 'lucide-react';
import type { GameMode, GameModeDetails, WordData } from './types'; // Ensure types.ts is created
import { motion, AnimatePresence } from 'framer-motion';
import { cn } from '@/lib/utils';

const gameModesDetails: Record<GameMode, GameModeDetails> = {
  junior: { title: 'Junior Journey', description: 'Pictures & Simple Words', icon: Lightbulb },
  basic: { title: 'Basic Builder', description: 'Everyday Words - MCQs', icon: Lightbulb },
  intermediate: { title: 'Intermediate Challenge', description: 'Fill-in-the-blanks', icon: Lightbulb },
  advanced: { title: 'Advanced Arena', description: 'Descriptive Clues - Timed', icon: Lightbulb },
  rapidFire: { title: 'Rapid Fire Round', description: 'Quick Meanings Match', icon: Lightbulb },
};

// Placeholder word data - replace with actual data source
const MOCK_WORDS: Record<GameMode, WordData[]> = {
  ...
  rapidFire: [ // Simple word-meaning pairs for rapid fire
    { id: 'rf1', word: 'Fast', clueType: 'meaning', clue: 'Quick', options: ['Quick', 'Slow', 'Heavy', 'Light'], correctAnswer: 'Quick', matchWord: 'Fast' },
    { id: 'rf2', word: 'Large', clueType: 'meaning', clue: 'Big', options: ['Small', 'Big', 'Tall', 'Short'], correctAnswer: 'Big', matchWord: 'Large'},
    { id: 'rf3', word: 'Joyful', clueType: 'meaning', clue: 'Happy', options: ['Sad', 'Angry', 'Happy', 'Calm'], correctAnswer: 'Happy', matchWord: 'Joyful'},
  ],
};
const MAX_TIME_PER_QUESTION = 30; // seconds, adjust per mode if needed for Advanced/RapidFire
...
export default function GameInterface({ selectedMode, onGoBack }: GameInterfaceProps) {
  ...
  const resetTimer = useCallback(() => {
    ...
    if (selectedMode === 'rapidFire') duration = 10;
    setTimeLeft(duration);
  }, [selectedMode]);

  // Shuffle options for MCQ modes
  useEffect(() => {
    if (currentWordData && (selectedMode === 'basic' || selectedMode === 'rapidFire') && currentWordData.options) {
  ...
  const handleSubmitAnswer = () => {
    ...
    let isCorrect = false;
    if (selectedMode === 'basic' || selectedMode === 'rapidFire') {
      isCorrect = userInput === currentWordData.correctAnswer; // UserInput here would be the selected option
    } else {
      isCorrect = userInput.trim().toLowerCase() === currentWordData.correctAnswer.toLowerCase();
    }

    if (isCorrect) {
      setScore(s => s + 10); // Example scoring
      setFeedbackMessage('Correct! 🎉');
    } else {
      setFeedbackMessage(`Oops! The correct answer was: ${currentWordData.correctAnswer}`);
    }

    // Wait a bit to show feedback, then move to next question or end game
    setTimeout(() => {
  ...
  const handleOptionSelect = (option: string) => {
      setUserInput(option); // For MCQ, userInput stores the selected option
      // Automatically submit for MCQ type for faster gameplay
      // We need to ensure currentWordData is available.
      if (currentWordData) {
    ...
    return (
      <Card className="w-full max-w-lg mx-auto text-center shadow-xl animate-in fade-in-50">
        <CardHeader>
          ...
          <CardDescription>Mode: {gameModesDetails[selectedMode].title}</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardFooter className="flex flex-col sm:flex-row gap-2 justify-center">
          ...
          {/* <Button>View Leaderboard (Soon)</Button> */}
        </CardFooter>
      </Card>
    );
  }
  ...
  const renderAnswerArea = () => {
    if (selectedMode === 'basic' || (selectedMode === 'rapidFire' && currentWordData.options && currentWordData.options.length > 0)) {
      return (
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-4 sm:space-y-6">
      ...
      <motion.div
        key={currentWordIndex} // Animate when question changes
        ...
        <Card className="shadow-xl overflow-hidden">
          <CardHeader className="p-4 sm:p-5 bg-muted/50">
            <div className="flex justify-between items-center">
              <CardTitle className="text-xl sm:text-2xl text-primary">{gameModesDetails[selectedMode].title}</CardTitle>
          ...
          <CardContent className="p-4 sm:p-6 grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="space-y-3 sm:space-y-4 flex flex-col justify-between">
              <div>
                {(selectedMode === 'intermediate' || selectedMode === 'advanced') && (
                  <div className="mb-3 p-2 border-dashed border-2 rounded-md text-center text-sm sm:text-lg tracking-[0.2em] font-mono h-10 sm:h-12 flex items-center justify-center bg-muted/30">
                    {/* Placeholder for animated blanks, for now show hint or word length */}
              ...
              <div className="space-y-2">
                {currentWordData.hint && (selectedMode === 'intermediate' || selectedMode === 'advanced') && (
